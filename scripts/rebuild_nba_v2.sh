#!/usr/bin/env bash
# NBA surface rebuild after the 2026-07-11 giannis persona sharpening.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
cd "$ROOT"
mkdir -p artifacts/nba/v1-backup
for f in response_surface_zh.jsonl response_surface_en.jsonl response_surface_zh.npz response_surface_en.npz infer_table_zh.npz infer_table_en.npz; do
  [ -f "artifacts/nba/$f" ] && mv "artifacts/nba/$f" "artifacts/nba/v1-backup/$f"
done
